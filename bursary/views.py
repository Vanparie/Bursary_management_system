...
def apply_bursary(request):
    ...
    if request.method == "POST":
        ...
        guardian_form = GuardianForm(request.POST, instance=guardian, student=student)
        ...
        guardian = Guardian.objects.filter(student=student).first()
        guardian_form = GuardianForm(instance=guardian, student=student)
...
def signup_view(request):
    ...
    constituency_name = site_profile.constituency.name if site_profile.constituency else None

    if not county_name and not constituency_name:
        messages.error(request, "❌ System misconfiguration: No county or constituency set for this bursary site.")
        return redirect("student_login")
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            # Determine which name and ID to verify based on the type
            if id_type == "nemis":
                id_number = form.cleaned_data.get("guardian_id_number")
            ...
            # Call verify_id and unpack all expected values properly
            is_valid, message, verified_county, verified_constituency, verified_name = verify_id(
                id_number=id_number,
            ...
            # Prefill and lock guardian details if verified successfully
            if id_type == "nemis":
                form.fields["guardian_first_name"].initial = verified_name.split()[0] if " " in verified_name else verified_name
                form.fields["guardian_last_name"].initial = (
                    verified_name.split()[1] if " " in verified_name else ""
                )
                form.fields["guardian_first_name"].widget.attrs["readonly"] = True
                form.fields["guardian_last_name"].widget.attrs["readonly"] = True

            # Save student, binding them to the verified region
            student = form.save(commit=True)
            student.county = site_profile.county or None
            student.constituency = site_profile.constituency or None
            student.save()
...
def verify_identity_ajax(request):
    """
    Unified AJAX endpoint to verify either Student or Guardian ID.
    Uses mock national database, validates name + ID combination,
    and enforces county/constituency match based on active site profile.
    """
    id_type = request.POST.get("id_type", "id")  # 'id' for student, 'guardian' for guardian
    id_number = request.POST.get("id_number")
    first_name = request.POST.get("first_name")
    last_name = request.POST.get("last_name")

    site = getattr(request, "site_profile", None)
    county_name = site.county.name if site and site.county else None
    constituency_name = site.constituency.name if site and site.constituency else None

    try:
        is_valid, message, county, constituency, verified_name = verify_id(
            id_number=id_number,
            first_name=first_name,
            last_name=last_name,
            county_name=county_name,
        ...
        if is_valid:
            response_data = {
                "valid": True,
                "message": f"{'Guardian' if id_type == 'guardian' else 'Student'} ID verified successfully.",
                "verified_county": county,
                "verified_constituency": constituency,
        ...
        return JsonResponse(
            {"valid": False, "message": f"⚠️ Network error during verification: {e}"},
            status=500,
        )
...
def student_login_view(request):
    site = getattr(request, 'site_profile', None)  # active site profile
...
# ========================
# Export Applications as PDF (with Logo, Smart Header & Footer + Page Numbers + Wrapped Text)
# ========================
@login_required
def export_applications_pdf(request):
    ...
    from bursary.models import SiteProfile, OfficerProfile, BursaryApplication, Guardian

    # --- Determine Officer Context ---
    officer = getattr(request.user, "officerprofile", None)
    area_label = "Bursary Office"
    logo_path = None

    if officer:
        if officer.constituency:
            area_label = f"{officer.constituency.name} Constituency Bursary Office"
        elif officer.county:
            area_label = f"{officer.county.name} County Bursary Office"
    else:
        site_profile = SiteProfile.objects.first()
        if site_profile:
            if getattr(site_profile, "constituency", None):
                area_label = f"{site_profile.constituency.name} Constituency Bursary Office"
            elif getattr(site_profile, "county", None):
                area_label = f"{site_profile.county.name} County Bursary Office"
            else:
                area_label = "Bursary Office"

            if getattr(site_profile, "branding_logo", None):
                logo_path = site_profile.branding_logo.path
    ...
    normal_wrap = ParagraphStyle(
        ...
        leading=8,
        alignment=1,  # center
        wordWrap='CJK',  # enables word wrapping for long content
    )
    ...
    elements.append(Paragraph(area_label, title_style))
    elements.append(Spacer(1, 8))
    elements.append(Paragraph("<b>Bursary Applications Report</b>", styles["Title"]))
    elements.append(Spacer(1, 12))

    # --- Table Data ---
    headers = [
        "Full Name", "Adm No", "ID No", "Institution", "Course",
        "Year", "Constituency", "Requested", "Awarded",
        "Status", "Phone", "Email", "Guardian", "Income", "Date"
    ]

    data = [[Paragraph(h, normal_wrap) for h in headers]]
    ...
    for app in applications:
        ...
        guardian = guardian_map.get(student.id)
        row = [
            Paragraph(str(student.full_name), normal_wrap),
            Paragraph(str(student.admission_number), normal_wrap),
            Paragraph(str(student.id_number), normal_wrap),
            Paragraph(str(student.institution), normal_wrap),
            Paragraph(str(student.course), normal_wrap),
            Paragraph(str(student.year_of_study or ""), normal_wrap),
            Paragraph(str(app.constituency.name if app.constituency else ""), normal_wrap),
            Paragraph(f"{app.amount_requested or 0}", normal_wrap),
            Paragraph(f"{app.amount_awarded or 0}", normal_wrap),
            Paragraph(app.status.title(), normal_wrap),
            Paragraph(str(student.phone), normal_wrap),
            Paragraph(str(student.email), normal_wrap),
            Paragraph(str(guardian.name if guardian else ""), normal_wrap),
            Paragraph(str(guardian.income if guardian else ""), normal_wrap),
            Paragraph(app.date_applied.strftime("%Y-%m-%d"), normal_wrap),
        ]
        data.append(row)

    # --- Adjusted Column Widths ---
    col_widths = [
        70, 45, 45, 85, 70, 30, 70, 45, 45, 50, 55, 65, 60, 60, 60
    ]
    ...
    # --- Footer (Officer Info + Date) ---
    if officer:
        role = "Constituency Officer" if officer.constituency else "County Officer"
        generated_by_text = f"Generated by: {officer.user.get_full_name() or officer.user.username} ({role})"
    elif request.user.is_authenticated:
        generated_by_text = f"Generated by: {request.user.get_full_name() or request.user.username}"
    else:
        generated_by_text = "Generated by: System Administrator"
    ...
    elements.append(Paragraph(generated_by_text, footer_style))
    ...
    def add_page_number(canvas, doc):
        ...
        canvas.drawRightString(800, 20, text)  # Adjust for landscape A4
